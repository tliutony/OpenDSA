...
The ``switch`` statement is not used very often, and is generally not recommended.
First, it is not as powerful as the ``else if`` model
...
.. code-block:: java
    ...
    int b = 20;
    if ((a > b) && (b > 0)) {
        System.out.println("a is greater than b and b is positive");
    }
...
regular if else statement, but sometimes it can be more convenient to use a single expression if the condition is simple.
