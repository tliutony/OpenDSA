...
called "hello world."" There are no logic errors to make, so getting it to run relies only on
...
If you are familiar with the Python interpreter, you can define this program right at the Python command
...
with an editor. It is in a format that is not human-readable, but is "computer-readable".
...
.. note::
    It is important to know how to compile and run a Java program manually from the command line, but 
    also know that VS Codium provides the convenient "Run" button that compiles and runs your program automatically. 
    You'll get experience with both ways of running your programs this semester in lab.

1. Every Java program must define a class, and all code is inside a class

2. Everything in Java must have a type

Let's take the hello world example a line at a time to see how these
...
``{`` and ends with a ``}``. You will notice that we indented the code
...
deeply right away -- we will unpack all of these keywords over the next few sections.
...
same way you think of methods in Python files that don't require an
object instance to be called.
...
letters and numbers. Names in Java must start with a letter.
...
statements are all legal and equivalent. We would not encourage you to
...
We will continue to unpack these keywords and their meanings over the next few classes!